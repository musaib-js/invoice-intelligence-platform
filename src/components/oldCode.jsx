...
const Table = ({
  ...
  const handleSaveClick = async()=>{
    let payload =  respData
    console.log("the data for editable table is", dataForEditabletable)
    console.log("data of invoice table is", invTableheaders)
    const tableDataNew = dataForEditabletable.unshift(invTableheaders);
    console.log("the table data new is", tableDataNew)
    const reversedData = {};
    const keys = Object.keys(tableDataNew[0]);
    for (const key of keys) {
      reversedData[key] = [];
    }
    for (const obj of tableDataNew) {
      for (const key of keys) {
        reversedData[key].push(obj[key]);
      }
    }
    payload["invoice_1"] = reversedData
    console.log("the temp payload is", payload)
  }
  ...
  return (
    <>
      ...
      {showTable ? (
        ...
        //       Reject
        //     </button>
        //     <button className="shadow-lg btn mx-1 btn-sm btn-warning" onClick={()=>handleSaveClick()}>
        //       Save
        //     </button>
        //   </div> 
        // </>
